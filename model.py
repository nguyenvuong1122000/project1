...
class LSTM_Cell(nn.Module):
    ...
    def init_hidden(self, batch_size, hidden, feauture_size):
        ...
        return (Variable(torch.zeros(batch_size, hidden, feauture_size)).cuda(),
                Variable(torch.zeros(batch_size, hidden, feauture_size)).cuda()
                )


class LSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, feauture_size, width, batch_size):
...
class G2RL(nn.Module):
    def __init__(self, N, input_channels):
        super(G2RL, self).__init__()
        self.backbone_CNN = backbone_CNN()
        self.RNN_model = LSTM(input_channels)
